...
def supervised_tokenize_sft(
    ...
    for mess in messages:
        ...
        template.messages.append((from_str, mess["value"]))
    ...
    tokenized = [tokenizer.bos_token_id] + tokenized # +1 and +2 due to this
    ...
    # print("labels_decode:", labels_decode)  # 调试用
...
class ConstantLengthSplicedDataset(IterableDataset):
    """
    Define an iterable dataset that returns a data point filled to max_length 
    spliced from multiple original independent (pre-tokenized) data points.
    This version uses real data from subsequent data points to fill the current data point.
    """
    ...
    def __iter__(self) -> Iterable[Dict[str, List[int]]]:
        ...
        while more_data_points is True:
            ...
            for i, data_point in enumerate(buffer):
                ...
                # Pre action judgment to fill the sequence to max_length
                if len(spliced_input_ids) + len(seq_input_ids) > self.max_length:
                    # Compute remaining space in current spliced data point
                    remaining_space = self.max_length - len(spliced_input_ids)
                    
                    # Use part of the current sequence to fill the remaining space
                    spliced_input_ids.extend(seq_input_ids[:remaining_space])
                    spliced_labels.extend(seq_labels[:remaining_space])
                    
                    # Add the current spliced data point to the examples
                    spliced_data_point = {
                        self.input_ids_field: spliced_input_ids,
                        self.labels_field: spliced_labels,
                    }
                    ...
                    # Reset for next splicing
                    spliced_input_ids, spliced_labels = [], []

                    # If the current sequence has leftover, add it to the next batch
                    remaining_seq_input_ids = seq_input_ids[remaining_space:]
                    remaining_seq_labels = seq_labels[remaining_space:]

                    spliced_input_ids.extend(remaining_seq_input_ids)
                    spliced_labels.extend(remaining_seq_labels)
            ...
            # For residual spliced data point at the end of the data set
            if self.infinite is False and more_data_points is False and len(spliced_input_ids) > 0:
                # Ensure the last batch is filled to max_length
                if len(spliced_input_ids) < self.max_length:
                    remaining_space = self.max_length - len(spliced_input_ids)
                    spliced_input_ids.extend(buffer[0][self.input_ids_field][:remaining_space])
                    spliced_labels.extend(buffer[0][self.labels_field][:remaining_space])
