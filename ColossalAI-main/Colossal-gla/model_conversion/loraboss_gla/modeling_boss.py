...
class BoSsLoRA2(nn.Linear):
    # LoRA implemented for both BoSs and Main parts, thus LoRA^2
    def __init__(
        self, 
        in_features: int, 
        out_features: int, 
        ...
        **kwargs
        ...
        self.lora_A_boss = nn.Parameter(self.weight.new_zeros((r, in_features)))
        self.lora_A_main = nn.Parameter(self.weight.new_zeros((r, in_features)))
        self.lora_B_boss = nn.Parameter(self.weight.new_zeros((out_features, r)))
        self.lora_B_main = nn.Parameter(self.weight.new_zeros((out_features, r)))
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, 'lora_A'):
            ...
            nn.init.kaiming_uniform_(self.lora_A_boss, a=math.sqrt(5))
            nn.init.kaiming_uniform_(self.lora_A_main, a=math.sqrt(5))
            nn.init.zeros_(self.lora_B_boss)
            nn.init.zeros_(self.lora_B_main)

    def forward(self, x: torch.Tensor, part: str):
        result = F.linear(x, self.weight, bias=self.bias)            
        if part == 'boss':
            result += (self.lora_dropout(x) @ self.lora_A_boss.transpose(0, 1) @ self.lora_B_boss.transpose(0, 1)) * self.scaling
        elif part == 'main':
            result += (self.lora_dropout(x) @ self.lora_A_main.transpose(0, 1) @ self.lora_B_main.transpose(0, 1)) * self.scaling
        else:
            raise ValueError("BoSs LoRA layer has only tow parts: boss and main")
        return result

class BoSsAttention(nn.Module):
    ''' Both BoSs part and Main Part are implemented using LoRA '''
    def __init__(self, config: BoSsConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = BoSsLoRA2(self.hidden_size, self.num_heads * self.head_dim, bias=False, r=64, lora_alpha=128)
        self.k_proj = BoSsLoRA2(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=False, r=64, lora_alpha=128)
        self.v_proj = BoSsLoRA2(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=False, r=64, lora_alpha=128)
        self.o_proj = BoSsLoRA2(self.num_heads * self.head_dim, self.hidden_size, bias=False, r=64, lora_alpha=128)
        
        # BoSs part parameters
        ...
        self.initialize_weights_ft()
        
    def initialize_weights_ft(self):
        # only gk needs to be initialized 
    ...
    def boss_attention(self, q, k, v, sid, sid_current):
        ...
        if sid_current is not None:
            ...
            ###################
            # idx = torch.argsort(sid, dim=1, descending=False, stable=True).to(dtype=torch.int64, device=sid.device)
            # state_sizes = torch.zeros(batch_size, ns_gen, dtype=torch.int64, device=sid.device)
            # for i in range(ns_gen):
            #     state_sizes[:, i] = (sid == i).sum(dim=1)
            # offs_k = torch.cat([torch.tensor([0], dtype=torch.int64, device=sid.device), state_sizes.flatten().cumsum(dim=0)])
            ###################
            ...
            ###################
            # ns = self.num_states if self.training else self.num_states + 1
            # idx = torch.argsort(sid, dim=1, descending=False, stable=True).to(dtype=torch.int64, device=sid.device)
            # state_sizes = torch.zeros(batch_size, ns, dtype=torch.int64, device=sid.device)
            # for i in range(ns):
            #     state_sizes[:, i] = (sid == i).sum(dim=1)
            # offs = torch.cat([torch.tensor([0], dtype=torch.int64, device=sid.device), state_sizes.flatten().cumsum(dim=0)])
            ###################
            idx = rearrange(idx, 'b l -> b l 1 1').contiguous().expand_as(v).to(torch.int64) # bs qlen -> bs qlen 1 1 -> [batch_size, q_len, num_heads, head_dim]
    ...
    def boss_part(
        ...
        query_states = self.q_proj(hidden_states, part='boss')
        key_states = self.k_proj(hidden_states, part='boss')
        value_states = self.v_proj(hidden_states, part='boss')
        ...
        attn_output = self.o_proj(attn_output, part='boss')
    ...
    def main_part(
        ...
        query_states = self.q_proj(hidden_states, part='main')
        key_states = self.k_proj(hidden_states, part='main')
        value_states = self.v_proj(hidden_states, part='main')
        ...
        attn_output = attn_output.reshape(bsz, q_len, self.num_heads * self.head_dim).contiguous()
        attn_output = self.o_proj(attn_output, part='main')
    ...
    def forward(
        ...
        boss_part, _, past_key_value = self.boss_part(
        ...
        main_part, _, past_key_value = self.main_part(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            past_key_value=past_key_value,
            use_cache=use_cache,
        )
        attn_output = main_part + boss_part * out_weight.to(boss_part.dtype).unsqueeze(-1)
        return attn_output, None, past_key_value
...
class BoSsDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        out_weight, state_indices = gates.max(dim=-1)
        if self.training:
            mask1 = F.one_hot(state_indices, num_classes=self.num_states)
        ...
        # Self Attention
        hidden_states, _, past_key_value = self.attn_boss(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            sid=state_indices,
...
class LoRA2BoSsForCausalLM(BoSsPreTrainedModel):
