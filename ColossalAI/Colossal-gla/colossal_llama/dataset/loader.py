...
def load_tokenized_dataset(
    dataset_paths: Union[PathType, List[PathType]], mode: str = "train", total_samples: Optional[int] = None
    ...
    for ds_path in dataset_paths:
        ...
        if _total_samples == 0:
            break
        if _total_samples is not None:
            if len(ds_dict) > _total_samples:
                ds_dict = ds_dict.select(range(_total_samples))
            else:
                _total_samples -= len(ds_dict)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict[str, List[int]]]) -> Dict[str, torch.Tensor]:
        ...
        #         print(f"Warning: the length of input_ids is not equal to {self.max_length}, len={len(instance['input_ids'])}")
        # `List[torch.Tensor]`
        batch_input_ids = [
            (
        ...
        num_of_ture = attention_mask.sum().item()
        # print(f"num_of_ture: {num_of_ture}")
