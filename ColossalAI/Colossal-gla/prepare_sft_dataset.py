...
import glob
from colossal_llama.dataset.conversation import LLaMA2_Conv, LLaMA3_Conv, Qwen
...
def main():
    ...
    # print(f"input_data_dirs: {input_data_dirs}")
    for ds_dir in input_data_dirs:
        # Use glob to match files with wildcard support
        matched_files = glob.glob(os.path.join(ds_dir, "*.jsonl"))
        if not matched_files:
            raise ValueError(f"No files matched the pattern {ds_dir}")
        input_data_paths.extend(matched_files)
    input_data_paths = sorted(input_data_paths)
    print(f"input_data_paths: {input_data_paths}")
    # return
    ...
    if args.llama_version == 4:
        default_conversation = Qwen
    ...
    # print(default_conversation.seps)
