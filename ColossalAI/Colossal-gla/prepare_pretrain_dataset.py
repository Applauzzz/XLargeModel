...
def preprocess_parquet_dataset(input_data_paths, tokenizer, max_length):
    ...
    for data_path in input_data_paths:
        ...
        # Assuming each row has 'id' and 'conversations' columns
        for index, row in df.iterrows():
            conversation = row['conversations']
            # Concatenate human and gpt conversations
            dialogue = " ".join([item['value'] for item in conversation])
            conversations_data.append({"dialogue": dialogue})

    # Create a HuggingFace dataset from processed conversations
    dataset = Dataset.from_pandas(pd.DataFrame(conversations_data))

    # Tokenization process
    tokenized_dataset = dataset.map(
        lambda e: tokenizer(e['dialogue'], truncation=True, padding='max_length', max_length=max_length),
        batched=True,
    )
...
def main():
    ...
    # input_data_paths = input_data_paths[:1]  # For test
    ...
    # list_dataset = preprocess_parquet_dataset(input_data_paths, tokenizer, args.max_length)
    for index, dataset in enumerate(list_dataset):
        ...
        spliced_dataset = ConstantLengthSplicedDataset(
