...
class mhBoSsAttention(nn.Module):
    def __init__(self, config: BoSsConfig, layer_idx: Optional[int] = None):
        ...
        self.router = nn.ModuleList([nn.Linear(self.head_dim, self.num_states) for _ in range(self.num_heads)])
    ...
    def forward(
        ...
        hidden_states = hidden_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        ...
        for head_idx in range(self.num_heads):
            gates = self.router[head_idx](hidden_states[:, head_idx, :, :])  # Get the gate
            gates = F.softmax(gates, dim=-1)
            
            # compute the balance loss
            out_weight, indices1_s = gates.max(dim=-1)
            out_weight_list.append(out_weight)
            sid_list.append(indices1_s)
            if self.training:
                mask1 = F.one_hot(indices1_s, num_classes=self.num_states)
                me = torch.mean(gates, dim=(0, 1))
                ce = torch.mean(mask1.float(), dim=(0, 1))
                l_aux = l_aux + torch.sum(me * ce) * self.num_states
        l_aux = l_aux / self.num_heads
        out_weight = torch.stack(out_weight_list, dim=-1)  # b l h
        sid = torch.stack(sid_list, dim=1)  # b h l
        sid = rearrange(sid, "b h l -> (b h) l")
        sid_current = sid.squeeze() if q_len == 1 else None
            
        # for generation
        # Now it cant be squeezed
        # sid_current = sid.squeeze() if q_len == 1 else None
        ...
        query_states = rearrange(query_states, "b h l d -> (b h) l 1 d")
        key_states = rearrange(key_states, "b h l d -> (b h) l 1 d")
        value_states = rearrange(value_states, "b h l d -> (b h) l 1 d")

        
        attn_output = self.boss_attention(query_states, key_states, value_states, sid, sid_current)
        attn_output = rearrange(attn_output, "(b h) l 1 d -> b l h d", h=self.num_heads) * out_weight.to(attn_output.dtype).unsqueeze(-1)
...
class mhBoSsDecoderLayer(nn.Module):
    def __init__(self, config: BoSsConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.layer_idx = layer_idx

        self.attn_boss = mhBoSsAttention(config=config, layer_idx=layer_idx)
    ...
    def forward(
        ...
        # Self Attention
        boss_part, _, past_key_value, l_aux = self.attn_boss(
        ...
        hidden_states = residual + main_part + boss_part
...
class mhBoSsPreTrainedModel(PreTrainedModel):
...
class mhBoSsModel(mhBoSsPreTrainedModel):
    """
    ...
    def __init__(self, config: BoSsConfig):
        ...
        self.layers = nn.ModuleList(
            [mhBoSsDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class mhBoSsForCausalLM(mhBoSsPreTrainedModel):

    def __init__(self, config):
        super().__init__(config)
        self.model = mhBoSsModel(config)
        self.vocab_size = config.vocab_size
